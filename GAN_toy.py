# a toy example showing how to learn a distribution using GANs
...
# our data follows a sigma distribution
...
class GeneratorDistribution(object):
    ...
    # stratified sampling -- uniform over range, then randomly perturbed
    def sample(self, N):
        return np.linspace(-self.range, self.range, N) + \
            np.random.random(N) * 0.01

# a nice and simple generator
def generator(input, h_dim):
    h0 = tf.nn.softplus(linear(input, h_dim, 'g0'))
    h1 = linear(h0, 1, 'g1')
    return h1
    
# a fancier discriminator (needed for late-stage training)
